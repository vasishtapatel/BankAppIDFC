...
function Deposit() {
  ...
  const submitHandler = (event) => {
    ...
    let response = fetch(
      `/deposit/${state.accNo}/${state.amount}/${state.ifscCode}/${state.custid}`,
      {
        method: "get",
        headers: {
    ...
    response
      .then((res) => {
        if (res.status === 200)
          alert(
            `Deposited amount ${state.amount} into account number ${state.accNo}.`
          );
    ...
    setState({ ...state, accNo: "", amount: "", ifscCode: "", custid: "" });
  };
      ...
      <form onSubmit={submitHandler} className="form">
        <div className="form__item">
          ...
          <input
            ...
            onChange={(event) =>
              setState({ ...state, accNo: event.target.value })
            }
          />
        </div>
        <div className="form__item">
